...
// }
// 800 = config.scale.width
// 600 = config.scale.height
...
{    
    ...
    for(var i=0; i<3; i++){
        ...
        // rank_cont[i].setDisplaySize(800 / 3, 600 / 9);
        // console.log(800, 600);
    ...
    for(i=0; i<5; i++){
        FireList[i] = this.physics.add.image(0, Random_Int(600, 10), 'Fire');
        FireList[i].setDisplaySize(60, 73);
        FireList[i].setCircle(28, FireList[i].width/2 - 25, FireList[i].height/2 - 28)
        FireList[i].angle = -90

        
        FireList[i+5] = this.physics.add.image(Random_Int(800, 10), 0, 'Fire');
...
{
    // <벽 통과 방지>
    if(player.x >= 800 - player.body.width / 2){
        player.x = 800 - player.body.width / 2;
    }
    else if(player.x <= player.body.width / 2){
        player.x = player.body.width / 2;
    }

    if(player.y >= 600 - player.body.height / 2){
        player.y = 600 - player.body.height / 2;
    }
